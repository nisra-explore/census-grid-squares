...
function handlePostcodeSearch() {
  ...
  const zoneType = zoneSelector ? zoneSelector.value : 'grid';
...
function fetchGridData() {
  ...
  const csvUrl = 'create-js\\inputs\\census_grid_square_data_light.csv';
  ...
  return fetch(csvUrl)
    ...
    .then(data => {
      ...
      rows.slice(1).forEach(row => {
        ...
        const geography = cols[geographyIndex]?.trim();

        if (geography) {
          gridDataCache[geography] = cols;
        }
      ...
      console.log(
        'Grid records loaded:',
        Object.keys(gridDataCache).length
      );
...
function getZoneIdsFor(zone) {
  ...
  if (zone === 'grid') return {
    source: 'grid',
    sourceLayer: 'Grid_Square_2021_1km',
    fillLayer: 'grid-fill'
  };
  return {
    source: 'grid',
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let currentZoneType = 'grid';
  const zoneSelector = document.getElementById('zone-selector');
  zoneSelector.value = 'grid';
  ...
  function onZoneChange(e) {
    ...
    console.log("Zone changed to:", selected);
    updateSourceLink();

    vis('sdz-fill', selected === 'sdz'); vis('dz-fill', selected === 'dz'); vis('dea-fill', selected === 'dea'); vis('lgd-fill', selected === 'lgd'); vis('grid-fill', selected === 'grid');
...
function updateSummaryPreview() {
  ...
  if (previewActiveZone !== z) {
    ...
    previewId: id,
    featureId,
    mapping: lgdNameToId.get(id)
  });
      try {
        previewMap.setFeatureState(
          {
            source: oldSource,
            sourceLayer: oldLayer,
            id: featureId
          },
          {
            hovered: false
  ...
  map.on('load', () => {
    ...
    map.addSource('grid', {
      type: 'vector',
      tiles: [
        'https://raw.githubusercontent.com/nisra-explore/map_tiles/main/census/{z}/{x}/{y}.pbf'
      ],
      promoteId: {
        'Grid_Square_2021_1km': 'GSQ21_1km'
      }
    });
...
map.on('error', (e) => {
  console.error('MAP ERROR', e);
    ...
    setTimeout(() => {
      ...
      const features = map.querySourceFeatures('grid', {
        sourceLayer: 'YOUR_LAYER_NAME'
      });
    ...
    map.addLayer({
      ...
      type: 'fill',
      source: 'grid',
      'source-layer': 'Grid_Square_2021_1km',
      paint: {
        'fill-color': '#3878c5',
        'fill-opacity': [
          'case',
...
// Click to select/deselect areas - LGD layer
    ...
    map.on('click', 'grid-fill', async (e) => {
      ...
      const gridId = feature.properties.GSQ21_1km;
  ...
  map.on('click', 'grid-fill', (e) => {
    ...
    function zoneIds() {
      ...
      if (activeZone === 'grid') return {source: 'grid', sourceLayer: 'Grid_Square_2021_1km', fillLayer: 'grid-fill' };
    ...
    function selectByGeometry(geom, mode = 'add') {
      ...
      const candidates = map.queryRenderedFeatures([sw, ne], { layers: [fillLayer] })
        .filter(f => {
          try {
            return turf.booleanWithin(f.geometry, feature.geometry);
          } catch {
            return false;
          }
        });
...
map.on('load', () => {
  ...
  map.setLayoutProperty('dea-fill', 'visibility', 'none');
  map.setLayoutProperty('lgd-fill', 'visibility', 'none');

  map.setLayoutProperty('sdz-outline-default', 'visibility', 'none');
  map.setLayoutProperty('dz-outline-default', 'visibility', 'none');
  map.setLayoutProperty('dea-outline-default', 'visibility', 'none');
  map.setLayoutProperty('lgd-outline-default', 'visibility', 'none');

  map.setLayoutProperty('sdz-outline-hover', 'visibility', 'none');
  map.setLayoutProperty('dz-outline-hover', 'visibility', 'none');
  map.setLayoutProperty('dea-outline-hover', 'visibility', 'none');
  map.setLayoutProperty('lgd-outline-hover', 'visibility', 'none');
  }); 
  ...
  onZoneChange({ target: zoneSelector });
  ...
  document.getElementById('zone-selector').addEventListener('change', function () {
    ...
    vis('sdz-outline-default', false);
    vis('dz-outline-default', false);
    vis('dea-outline-default', false);
    vis('lgd-outline-default', false);

    vis('sdz-outline-hover', false);
    vis('dz-outline-hover', false);
    vis('dea-outline-hover', false);
    vis('lgd-outline-hover', false);

    vis('grid-fill', true);
    vis('grid-outline-default', true);
    vis('grid-outline-hover', true);
...
function downloadSummaryImage() {
  ...
  if (view === 'charts' || view === 'chartComparison') {
    ...
    if (summaryHeroClone) {
      summaryHeroClone.style.gridTemplateColumns = 'repeat(2, minmax(0, 1fr))';
    ...
    if (summaryMapPanelClone) {
      ...
      summaryMapPanelClone.style.marginLeft = '-100px';
      summaryMapPanelClone.style.width = 'calc(100% + 16px)';
    }
...
async function downloadExcel() {
  ...
  const groups = ["Urban", "Rural", "Mixed"];
  const zoneType = window.selectedZoneType || 'grid';
  const zoneTypeText = zoneType === 'sdz'
    ? 'Super Data Zone'
    : zoneType === 'dz'
      ? 'Data Zone'
